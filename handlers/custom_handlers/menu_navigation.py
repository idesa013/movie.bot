...
@bot.message_handler(func=lambda m: (m.text or "").strip() in (_TEXT["en"]["favorites"], _TEXT["ru"]["favorites"]))
def open_favorites_menu(message: Message):
    ...
    bot.send_message(
        message.chat.id,
        "Выберите раздел:" if lang == "ru" else "Choose section:",
        reply_markup=favorites_menu(lang),
    )


@bot.message_handler(func=lambda m: (m.text or "").strip() in (_TEXT["en"]["recommendations"], _TEXT["ru"]["recommendations"]))
...
@bot.message_handler(func=lambda m: (m.text or "").strip() in (_TEXT["en"]["back"], _TEXT["ru"]["back"]))

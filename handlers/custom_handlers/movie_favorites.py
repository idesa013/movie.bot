...
def _movie_fav_limit() -> int:
    return get_config_int("qty_movie_fav", 30)


def _movie_fav_count(user_id: int) -> int:
...
def _build_movie_markup(
    viewer_id: int, target_user_id: int, movie_id: int
    ...
    # 4) Back to selected user for admin mode
...
def handle_favorites(call: CallbackQuery):
    ...
    if call.data.startswith("add_fav:"):
        ...
        bot.edit_message_reply_markup(
            ...
            reply_markup=_build_movie_markup(viewer_id, user_id, movie_id),
        )
    ...
    if call.data.startswith("remove_fav:"):
        ...
        bot.answer_callback_query(
            call.id,
            t(lang, "fav_removed").format(name=movie_name),
        )
        return
